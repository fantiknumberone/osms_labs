...
% Таблица автокорреляции как в C-коде
for s = 0:L-1
    fprintf('%2d | ', s);
...
% Дополнительно: вывод значений xcorr() и autocorr()
fprintf('\n=== Проверка через функции MATLAB ===\n');
fprintf('xcorr(): R(0)=%.6f, R(1)=%.6f\n', R_xc(lags_xc==0), R_xc(lags_xc==1));

try
    R_ac = autocorr(g_pm, 'NumLags', 30);
    fprintf('autocorr(): R(0)=1.000000, R(1)=%.6f\n', R_ac(2));
catch
    fprintf('autocorr() недоступна\n');
end